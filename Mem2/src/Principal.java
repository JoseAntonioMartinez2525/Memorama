...
public class Principal extends JFrame{
 ...
 public void agregarPaneles(){
  ...
  seleccionados.clear();
  for(int i=0;i<10;i++){
   ImageIcon img=new ImageIcon(this.getClass().getResource("/img/"+(i+1)+".png"));
 ...
 public void verificar() {
  if (seleccionados.size() == 3) {
   if (seleccionados.get(0).clave == seleccionados.get(1).clave) {
    ...
    ocultar();
    contadorganador++;
    JOptionPane.showMessageDialog(null, "Veces ganadas: "+contadorganador);
    
   } else {
    seleccionados.clear();
    ocultar();
    contadorperdedor++;
    JOptionPane.showMessageDialog(null, "Intentos fallidos: "+contadorperdedor);
